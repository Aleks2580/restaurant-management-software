...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    activeOrdersAverageCheck = Math.round(
      activeOrdersTotal / activeOrders.length
    );
    const activeOrdersTables = activeOrders.length;

    const totalOrdersFromBack = await Order.findAll({ raw: true });
    const totalOrders = totalOrdersFromBack.filter(
      (order) =>
        order.createdAt.toDateString() === new Date(today).toDateString()
    );
    ...
    const totalOrdersTables = totalOrders.length;

    const paidOrders = totalOrdersFromBack.filter(
      (order) =>
        order.createdAt.toDateString() === new Date(today).toDateString() &&
        order.open === false &&
    ...
    totalPaidOrdersAverageCheck = Math.round(
      totalPaidOrdersTotal / paidOrders.length
    );
    let totalPaidOrdersTables = paidOrders.length;

    const reservationsFromBack = await Reservation.findAll({ raw: true });
    const reservations = reservationsFromBack.filter(
      (reservation) => new Date(reservation.date).setHours(0, 0, 0, 0) === today
    );
